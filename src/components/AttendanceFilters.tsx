...
const AttendanceFilters = ({
  ...
  // Compute available subjects dynamically based on selected Class
  ...
  const filteredSubjects = subjects.filter(s => s.classId.toString() === classId);

  const activeSubjectDetails = filteredSubjects.find(s => s.subjectId.toString() === subjectId);
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 w-full p-4 bg-lamaPurpleLight/50 rounded-md border border-lamaPurple mb-4 items-end shadow-sm">
        ...
        <div className="relative">
          ...
          >
            <span className="truncate">
              {subjectId === "whole_day" ? "⚡ All Subjects (Whole Day)" : activeSubjectDetails ? activeSubjectDetails.subject.name : "Choose Scope..."}
            </span>
          ...
          {isDropdownOpen && (
            <div className="absolute top-full mt-1 w-full bg-white border border-gray-200 rounded-md shadow-2xl z-[100] max-h-[300px] overflow-y-auto animate-in fade-in slide-in-from-top-1 duration-200">
              {canSeeAllDay && date && classId && (
                <div 
                  onClick={() => { setSubjectId("whole_day"); setIsDropdownOpen(false); }}
                ...
                >
                  ⚡ All Subjects (Whole Day Registry)
                </div>
              )}
              {filteredSubjects.map((sub, idx) => (
                <div
                  key={`${sub.subjectId}-${idx}`}
                  onClick={() => { setSubjectId(sub.subjectId.toString()); setIsDropdownOpen(false); }}
                ...
                >
                  <span className="font-semibold text-gray-700">{sub.subject.name}</span>
                  <span className="text-[10px] text-gray-400">Taught by: {sub.teacher.name.charAt(0)}. {sub.teacher.surname}</span>
                  ...
                  <div className="absolute left-full ml-2 top-0 hidden group-hover:flex flex-col gap-1.5 bg-gray-800/95 backdrop-blur-md text-white text-[10px] p-3 rounded-md shadow-2xl z-[110] w-[220px] pointer-events-none border border-white/5">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-right">{sub.subject.name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-400">Teacher:</span> 
                      <span className="font-medium text-right">{sub.teacher.name} {sub.teacher.surname}</span>
                    </div>
                    <div className="flex justify-between mt-1 pt-1 border-t border-gray-600/30">
                      <span className="text-gray-400 font-semibold italic text-[8px]">Selection ID: #{sub.subjectId}</span>
                    </div>
                  </div>
                </div>
              ))}
              {filteredSubjects.length === 0 && !canSeeAllDay && (
                <div className="p-4 text-center text-xs text-gray-400 italic">No assigned subjects found.</div>
              )}
            </div>
          )}
        </div>
      ...
      <div className="w-full md:w-auto flex justify-end">
         <button 
           onClick={() => { setClassId(""); setSubjectId(""); setDate(new Date().toISOString().split("T")[0]); }}
